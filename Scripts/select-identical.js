...
action.canPerformWithContext= function(context, outError) {
	...
	var prevChar = (range.location > 0 ? context.substringWithRange(new Range(range.location - 1, 1)) : ''),
		nextChar = (range.location + range.length < context.string.length ? context.substringWithRange(new Range(range.location + range.length, 1)) : '');
...
action.performWithContext= function(context, outError) {
	...
	if (range.length > 0) {
		...
		if ((range.location + range.length < context.string.length && re_wordChar.test(context.substringWithRange(new Range(range.location + range.length, 1)))) || (range.location - 1 < 0 && re_wordChar.test(context.substringWithRange(new Range(range.location - 1, 1)))) || !re_wordChar.test(text.substr(0, 1)) || !re_wordChar.test(text.substr(text.length - 1, 1))) {
			// There is either a word character immediately to the right or left of the selection or the first or last character is not a word character, so we can't use boundaries
			useWordBoundaries = false;
		}
	} else {
		// Find the word under the cursor
		text = getWordAtCursor(context);
	}
	var searchLength = text.length;
...
var getWordAtCursor = function(context) {
	...
	function endsWithTag(currentIndex) {
		var linestart = context.lineStorage.lineStartIndexLessThanIndex(currentIndex),
			range = new Range(linestart, currentIndex - linestart + 1),
			text = context.substringWithRange(range);
